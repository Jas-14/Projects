...
class Paragraphe:
    ...
    def CreerPara(self, pot, title, content, statut, c, hist):
        ...
        if (self.position == -1):
            ...
            del (self) #Cette méthode permet de supprimer tout les attributs de la classe
            Button(error, text='OK', command=error.destroy).pack()
        else :
            hist.ajout_p(self)
            if (self.statut=="Fin Gagnante"):
                self.fin_g=True #On enregistre le type de fin dans un booléen
                self.statut='Fin' #On modifie le satut en un statut Fin général qui permet une optimisation des fonctions dans le reste du code
    ...
    def Affiche(self, b, hist,canvas):
        ...
        Button(paragraphe, text='Modifier titre', command=lambda:self.ModifierTitre(paragraphe, b, hist, canvas)).pack(side=LEFT)
    ...
    def Sauvegarder(self, texte, paragraphe):
        ...
        showinfo('Sauvegarde réussie','Sauvegarde réussie')

    def ModifierTitre(self, paragraphe, b, hist, canvas):
        nouveautitre=Tk()
        ...
        Button(nouveautitre, text='Valider', command=lambda:self.ValiderTitre(nouveautitre, title, b, hist, canvas)).pack(side=RIGHT)
        paragraphe.destroy()
        nouveautitre.mainloop()

    def ValiderTitre(self, nouveautitre, title, b, hist, canvas):
        self.titre=title.get()
        b['text']=self.titre #Change le textvariable du bouton par le nouveau titre
        nouveautitre.destroy()
        self.Affiche(b, hist, canvas)
    ...
    def Supprimer(self, b, paragraphe, hist,canvas):
        ...
        hist.del_p(self) #On enlève le référencement du paragraphe dans c_hsitoire
        for choix in self.choix: 
            del_attribut_c(self,choix,b,hist,canvas,0) #On supprime les choix liés au paragraphe

        for l in self.lien_I: #On supprime les liens liés aux choix du paragraphe de manière indirecte
            choix=l.link
            del_attribut_l(self,choix,b,hist,canvas,0,"p_O")
            
        del (self) #Cette méthode permet de supprimer tout les attributs de la classe
        b.destroy()
        paragraphe.destroy()

    def __del__(self): #Permet d'utiliser les del dans le programme
        return
